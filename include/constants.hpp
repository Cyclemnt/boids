#ifndef CONSTANTS_HPP
#define CONSTANTS_HPP

#include <cmath>

// Nombre de boids initiaux
#define NUM_BOIDS 65536 / 2
// Vitesse des boids
#define SPEED 30
#define ANG_V 2 * M_PIf
// Dimensions de l'environnement
#define ENV_WIDTH 1920
#define ENV_HEIGHT 1200
// Rayons d'interactions
#define R_DISTANCING 4.0f
#define R_ALIGNMENT 10.0f
#define R_COHESION 16.0f
// Fielf of view
#define FOV 5.0f
// Poids des interactions
#define WEIGHT_DISTANCING 0.05f
#define WEIGHT_ALIGNMENT 0.05f
#define WEIGHT_COHESION 0.0005f
// Temps entre chaque calculs
#define TIME_STEP 0.03f

// Ne pas changer
// Moitié des dimensions de l'environnement
#define HALVED_ENV_WIDTH ENV_WIDTH / 2
#define HALVED_ENV_HEIGHT ENV_HEIGHT / 2
// Rayons au carré
#define R_DISTANCING_SQUARED R_DISTANCING * R_DISTANCING
#define R_ALIGNMENT_SQUARED R_ALIGNMENT * R_ALIGNMENT
#define R_COHESION_SQUARED R_COHESION * R_COHESION
// Moitié de fov
#define HALVED_FOV FOV / 2.0f
// 2 pi
#define TWO_PIf 2 * M_PIf

#endif // CONSTANTS_HPP
